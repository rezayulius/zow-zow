...
<section id="health" class="relative py-16 overflow-hidden bg-white">
    ...
    <div class="absolute bottom-20 right-10 w-72 h-72 bg-pistache-100/50 rounded-full blur-3xl animate-pulse-slow"></div>
    <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-mint-50/40 rounded-full blur-3xl"></div>
    ...
    <div class="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @forelse($healthServices as $index => $service)
                ...
                <!-- Health Card: Clean, Vertical, Standard Grid -->
                <div class="group relative bg-white rounded-[2rem] p-5 border border-forest-moss-green-100 hover:border-forest-moss-green-300 shadow-lg shadow-forest-moss-green-50 hover:shadow-2xl hover:shadow-forest-moss-green-200/30 transition-all duration-500 hover:-translate-y-2 flex flex-col h-full overflow-hidden">
                    ...
                    <div class="relative w-full aspect-[4/3] rounded-[1.5rem] overflow-hidden mb-6 group-hover:rotate-1 transition-transform duration-500">
                        ...
                        <div class="absolute top-3 left-3">
                            <span class="bg-white/95 backdrop-blur-md text-forest-moss-green-800 text-[10px] font-bold px-3 py-1.5 rounded-full uppercase tracking-wider shadow-sm border border-white/20">
                                {{ $badge }}
                            </span>
                        </div>
                    </div>
                    ...
                    <!-- Content Area -->
                    <div class="px-2 pb-2 flex flex-col flex-grow text-center">
                        <h3 class="text-xl font-bold text-forest-moss-green-900 font-heading leading-tight mb-3 group-hover:text-forest-moss-green-600 transition-colors">
                            {{ $service->title }}
                        </h3>
                        
                        <p class="text-forest-moss-green-700/80 text-sm leading-relaxed mb-6 line-clamp-2">
                            {{ $service->description }}
                        </p>

                        <div class="bg-forest-moss-green-50/50 p-4 rounded-2xl border border-forest-moss-green-100/50 mb-6 text-left">
                            @foreach(array_slice($features, 0, 2) as $feature)
                                <div class="flex items-start gap-3 text-xs text-forest-moss-green-800 mb-2 last:mb-0">
                                    <div class="mt-0.5 w-4 h-4 rounded-full bg-forest-moss-green-200 flex items-center justify-center flex-shrink-0">
                                        <i data-lucide="check" class="w-2.5 h-2.5 text-forest-moss-green-700"></i>
                                    </div>
    ...
    <div class="max-w-7xl mx-auto px-6 relative z-10 pt-10">
        ...
        <div class="columns-1 md:columns-2 lg:columns-3 gap-8 space-y-8">
            @forelse($wellnessServices as $index => $service)
                ...
                <!-- Wellness Card: Organic, Masonry, Flowing -->
                <div class="group relative bg-white break-inside-avoid rounded-[3rem] rounded-tr-none p-4 border border-chai-100 hover:border-chai-300 shadow-xl shadow-chai-100/30 hover:shadow-2xl hover:shadow-chai-200/50 transition-all duration-500 hover:-translate-y-2 flex flex-col overflow-hidden">
                     ...
                     <div class="relative h-72 -mx-4 -mt-4 mb-6 overflow-hidden rounded-[3rem] rounded-tr-none">
                        ...
                        <div class="absolute bottom-4 left-4 right-4 flex justify-between items-end">
                             <h3 class="text-2xl font-bold text-white font-heading leading-tight shadow-black/50 drop-shadow-md">
                                {{ $service->title }}
                            </h3>
                            <div class="w-10 h-10 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center border border-white/30 group-hover:bg-white group-hover:text-chai-600 transition-all duration-300 animate-pulse-slow">
                                <i data-lucide="{{ $service->icon ?: 'heart' }}" class="w-5 h-5 text-white group-hover:text-chai-600"></i>
                            </div>
                        </div>

                        <div class="absolute top-4 left-4">
                            <span class="text-[10px] font-bold text-white uppercase tracking-widest bg-black/30 backdrop-blur-md px-3 py-1 rounded-full border border-white/20">{{ $badge }}</span>
                        </div>
                    </div>
                    ...
                    <div class="px-2 pb-4 flex flex-col flex-grow">
                        ...
                        <div class="space-y-2 mb-6">
                            @foreach($features as $feature)
                                <div class="flex items-center gap-3 text-sm text-chai-800">
                                    <div class="w-1.5 h-1.5 rounded-full bg-chai-400"></div>
                                    <span class="font-medium italic">{{ $feature }}</span>
                                </div>
                            @endforeach
                        </div>
...
<!-- Booking & Doctors Section -->
<section id="booking" class="relative py-16 bg-white overflow-hidden">
    <!-- Top Wave Separator -->
